...
Les fonctionnalités proposées permettent notamment le suivi et la comparaison de version et un reviewing efficace, traçable et transparant.

L'usage de Git est par ailleurs encore peu répandu en SHS. Nous souhaitons ainsi promouvoir son usage par l'intermédiaire du processus de publication de fiches Rzine. 
...
Le fonctionnement de Git repose sur des lignes de commande et, vous l'aurez compris, il s'agit du coeur autour duquel se sont développés des outils comme Github et Gitlab.
...
Cette page recense tous les répertoires associés à votre compte, si vous venez de vous créer un compte, elle devrait donc être vide, sinon vous retrouverez ici tous vos différents dépôts. Pour en créer un, il suffit de cliquer sur _new_.
...
Vous avez donc réalisé en local votre [fiche Rzine](https://rzine.gitpages.huma-num.fr/help_rzine_publication/), et disposez d'un dossier contenant tous les éléments requis.
...
Dans un second temps, il faut réaliser un _commit_^[Le __commit__ est l'enregistrement des changements effectués sur un fichier. Il s'agit de l'"instantané", du point de sauvegarde des changements qui ont été réalisés. C'est sur le commit que repose complétement le système de contrôle de version et donc plus largement les outils comme Git et Github. A chaque commit doit être associé un message en langue anglaise décrivant la nature de la modification apportée], c'est-à-dire une sauvegarde à un instant "t" des différents éléments du dossier et/ou de l'état des fichiers, grâce à la commande `git commit`.
...
> RY : Bonne pratique le terminal, mais je me demande si on n'a pas intérêt à faire la démonstration en mode clic-bouton. Par ailleurs pour avoir fait le test avec l'interface GitHub, tu peux importer plusieurs fichiers simultanément (procédure qui combine visiblement git add ., git commit -m"" et git push). Quitte à présenter la démarche en ligne de commande en annexes ?



# Gestion du processus d'évaluation avec Git

> J'ai créé ce paragraphe pour essayer de contextualiser le cycle de publication Rzine à la mode de Git... J'ai essayé de précsier les titres pour rappeler qui est concerné par quoi dans le cas d'un processus de publication Rzine... 

Une fois votre fiche Rzine soumise, l'évaluateur ([REVIEWER)] qui vous sera proposé va être amené à vous suggérer des améliorations de fond ou de forme  via des **issues** (commentaires) ou des propositions de correction / d'amélioration dans le corps de votre RMarkdown via des **pull request**, suivant les [consignes éditoriales de la collection Rzine](https://rysebaert.gitpages.huma-num.fr/rzine-editorial-guidelines/). 

Durant le processus de review, vous serez amené.e en tant que [REPO OWNER] à consulter et répondre à ces retours, puis éventuellement d'impacter les modifications suggérées dans votre fiche Rzine en cours d'évaluation. Le processus de review est publique (l'évaluateur n'est pas anonyme) et transparant (l'ensemble des modifications apportées au document étant historicisée).

Au préalable, vous recevrez une demande d'accès à votre dépôt GitHub ou GitLab pour que l'évaluateur puisse y déposer ses retours. Pour accorder l'accès à un collaborateur externe, se rendre dans *Settings*, puis *Manage Access* et saisir l'identifiant de l'évaluateur. Il aura alors le droit d'accéder à votre dépôt et vous proposer ses retours. 

Cette section décrit ce que sont les **issues** et les **pull request** ainsi que la procédure à suivre pour les prendre en compte. 


## Les "Issues" Github [REPO OWNER - REVIEWER]
...
## Branches et pull request [REVIEWER]

>  Est-ce qu'on ne va pas trop loin avec la création de branche ? Je me demande si on ne va pas perdre le lecteur ici, c'est un peu technique quand même ! Mais pourquoi pas après tout ! Est-ce que l'on demande donc au rewiewer de créer une branche pour y déposer ses pull requests ? 

Git et Github permettent de faire des branches^[Une __branche__ est une ramification indépendante de votre dépôt source. Elle permet donc de modifier en parallèle les fichiers ou la structure du dépôt sans affecter les autres branches. C'est extrêment utile pour travailler collaborativement sur un dépôt, d'autant qu'elles peuvent ensuite être re-fusionnées.]
...
Nous allons donc opérer les différentes modifications en local sur notre ordinateur. Une fois satisfaits, nous (**Le nous n'est pas clair, reviewer, repo owner ?**) allons créer une branche au sein de laquelle nous allons importer les fichiers modifiés sans écraser les premiers travaux. Le très grand intérêt à cela c'est que nous pourrons par la suite comparer les deux branches et éventuellement les fusionner.
...
> Peut être orienter davantage le propos autour du taf du reviewer, qui peut être amené à proposer ses retours au sein d'une ou plusieurs pull request que la personne qui soumet sera amenée à consulter ? 
...
> A partir de là, cela concerne la personne qui reçoit la review et qui doit accepter les commentaires, puis les traiter. Est-ce qu'il ne faudrait pas être plus précis sur les possibilités qui lui sont offertes ? Pour faciliter le processus, je suggèerais d'accepter les corrections, d'effectuer les modifications, de régénérer le html puis d'avertir le reviewer pour validation ?

## Merge pull request [REPO OWNER] 
...
## Synthétiser les modifications apportées [REPO OWNER - REVIEWER]

> RY : Je me demande s'il n'y a pas lieu de déposer à la racine du repo de la proposition un README.md qui synthétise les remarques principales de l'évaluateur et permet à la personne qui soumet de décrire les modifications apportées à sa fiche, d'où la proposition de ce paragraphe.

> J'ai tenté un [exemple bidon à partir d'une fiche bidon](https://github.com/rysebaert/ma_fiche_rzine/tree/main) 
